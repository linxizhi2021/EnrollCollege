...
class EnrollService extends BaseProjectService {
	...
	async getEnrollList({
		...
		let fields = 'ENROLL_STOP,ENROLL_JOIN_CNT,ENROLL_OBJ,ENROLL_VIEW_CNT,ENROLL_TITLE,ENROLL_MAX_CNT,ENROLL_START,ENROLL_END,ENROLL_ORDER,ENROLL_STATUS,ENROLL_CATE_NAME';
	...
	async getMyEnrollJoinList(userId, {
		...
		let fields = 'ENROLL_JOIN_LAST_TIME,ENROLL_JOIN_REASON,ENROLL_JOIN_ENROLL_ID,ENROLL_JOIN_STATUS,ENROLL_JOIN_ADD_TIME,enroll.ENROLL_TITLE';
	...
	async enrollJoin(userId, enrollId, forms) {
		...
		if (!enroll)
			this.AppError('该报名项目不存在或者已经停止');

		// 是否登记开始
		if (enroll.ENROLL_START > this._timestamp)
			this.AppError('该报名项目尚未开始');

		// 是否过了登记截止期
		if (enroll.ENROLL_END < this._timestamp)
			this.AppError('该报名项目已经截止');
	...
	async enrollJoinEdit(userId, enrollId, enrollJoinId, forms) {
		...
		let enrollJoin = await EnrollJoinModel.getOne(whereJoin);
		if (!enrollJoin)
			this.AppError('该报名项目不存在或者已经被系统取消');
